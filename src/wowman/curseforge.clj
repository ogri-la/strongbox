...
(defn-spec expand-summary (s/or :ok ::sp/addon, :error nil?)
  ...
  (info "downloading summary data for" (:name addon-summary))
  (let [detail-uri (str curseforge-host "/wow/addons/" (:name addon-summary))
        versions-uri (str detail-uri "/files")
        versions-html (html/html-snippet (utils/download versions-uri))
